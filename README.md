...
![planner](./presentation/planner.gif)

The final results and methodology are presented in this [presentation](./presentation/Large%20Scale%20Final%20Project.pptx). Below you can find the instructions that we were given for the final project.

## Executive summary: 

build a robust SBB journey planner for the Zürich area, and make a short video presentation of it, to be done in **groups of 5 or 6**.
...
Our implementation consists of 3 notebooks:
* `preprocessing.ipynb`: This notebook is used to preprocess the data and generate the graph used in the other notebooks.
* `algorithm.ipynb`: This notebook is used to implement the algorithm used to find a set of shortest paths between two nodes in a graph.
* `probabilistic_delay.ipynb`: This notebook is used to implement the probabilistic delay model and to compute the probability of success of a trip.

The robust journey planner interface can be found in the `algorithm.ipynb` notebook.

### Limitations

Predicting the probability of successful intercity travel using public transportation involves a complex array of factors, and even with the comprehensive assumptions and processes we have outlined, there are still potential limitations we consider. Here are a few:

* Timetable Change: Our model is based on the timetables of the week of April 17th, 2023. The assumption that these times remain static can be a potential limitation. Train and bus schedules can change due to many factors, such as holidays, construction, operational issues, etc. Changes may not always be reflected promptly in the SBB CFF's API, and could result in unexpected delays or cancellations, affecting the accuracy of the predictions.

* Walking Paths: The model assumes a straight-line walking path at a constant speed between stops. In reality, pedestrians have to navigate streets, terrain, and obstacles, which can affect the actual time required. Furthermore, weather conditions and personal factors (age, fitness, disabilities, etc.) can also influence walking speed.

* Radius Limitations: The model only considers stops within a 15 km radius of Zurich HB, possibly excluding relevant stations that could serve as optimal transfer points, particularly if the destinations are outside of this zone.

* Algorithmic Limitations: Yen’s algorithm may not find the optimal path in all cases, especially when considering complex multi-modal transit networks and time-dependent travel times. Additionally, limiting the output to the 100 shortest paths might exclude valid or preferable routes.

* Transfers are not independent: In reality, the success of one segment can directly influence the success of the next. For instance, if a train is delayed, it directly affects the passenger's ability to make a successful transfer to their next bus or train. This type of dependence isn't captured if all connections are assumed to be independent. By calculating the overall probability of a successful trip as the product of individual segment probabilities, errors in the estimation of individual segment probabilities can compound, potentially leading to overestimated or underestimated overall trip success probabilities.

* Ignoring Crowd Dynamics: Another factor is that each mode of transportation could influence the others in terms of passenger loads. For instance, a delayed train could lead to a surge of passengers on subsequent buses, increasing their likelihood of being full and therefore affecting the probability of a successful trip.
